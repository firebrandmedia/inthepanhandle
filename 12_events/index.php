		...
		<section class="body_content group">
			...
			<article class="m-all t-all da2-da4">
				...
				{exp:calendar:cal pad_short_weeks="n"
				   ...
				   <div class="header group">
				      <a href="{path='12_events/day'}{if segment_3 == "by_calendar"}by_calendar/{segment_4}/{/if}{prev_day format="%Y/%m/%d"}/"><i class="icon-left-open-big"></i></a>
				      <h2><span>({day_event_total})</span> Events for {day format="%D, %M %d, %Y"}</h2>
				      <a href="{path='12_events/day'}{if segment_3 == "by_calendar"}by_calendar/{segment_4}/{/if}{next_day format="%Y/%m/%d"}/"><i class="icon-right-open-big"></i></a>
				   </div>
				   {display_each_hour}
			...
			<footer class="m-all t-all da5-da6">
				<h3>Events by Month</h3>
				 <ul>
				{exp:calendar:month_list limit="12"}
