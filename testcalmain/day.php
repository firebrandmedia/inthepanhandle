{embed="testcalinc/index" page_title="Day View"}
</head>
<body id="home">
...
{exp:calendar:cal pad_short_weeks="n"
   ...
   <div class="header">
      <div class="left">
         <a href="{path='testcalmain/day'}{if segment_3 == "by_calendar"}by_calendar/{segment_4}/{/if}{prev_day format="%Y/%m/%d"}/">Yesterday</a>
      </div>
      <div class="center">
         <h2>{day format="%l, %F %d, %Y"} <span>({day_event_total} events)</span></h2>
      </div>
      <div class="right">
         <a href="{path='testcalmain/day'}{if segment_3 == "by_calendar"}by_calendar/{segment_4}/{/if}{next_day format="%Y/%m/%d"}/">Tomorrow</a>
      </div>
      <hr />
   </div>
   {display_each_hour}
...
{if event_multi_day == FALSE && event_all_day == FALSE}
   <div class="event">
         <h2><a href="{path='testcalmain/event'}{event_id}/">{event_title}</a> <span>({event_start_date format="%g:%i %a"}{if event_end_date}{if "{event_start_date format='%g:%i %a'}" != "{event_end_date format='%g:%i%a'}"} - {event_end_date format="%g:%i%a"}{/if}{/if})</span></h2>
...
{if:else}
   <div class="event_top">
         <h2><a href="{path='testcalmain/event'}{event_id}/">{event_title}</a> <span>({if event_multi_day == FALSE && event_all_day == TRUE}all day{if:else}{event_start_date format="%F %j, %Y at %g:%i%a"}{if event_end_date}{if "{event_start_date format='%g:%i%a'}" != "{event_end_date format='%g:%i%a'}"} - {event_end_date format="%F %j, %Y at %g:%i%a"}{/if}{/if}{/if})</span></h2>
         <p><b>From Calendar:</b> <a href="{path='testcalmain/calendars'}{event_calendar_url_title}/">{event_calendar_title}</a> &nbsp; &nbsp; <b>Location:</b> {event_location} &nbsp; &nbsp; <a href="{path='testcalmain/event'}{event_id}/">View Details</a></p>
   </div>
{/if}
   {/events}
   {/display_each_hour}
